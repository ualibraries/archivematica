...
#useradd www-data --uid 330 --gid 330 --create-home --home /var/lib/nginx --shell /bin/false
...
# mysql-server used by: archivematica-storage-service, archivematica-mcp-server, archivematica-mcp-client
# python-minimal gearmon used by: archivematica-storage-service, archivematica-mcp-server
# nginx used by archivematica-storage-service, dashboard
# archivematica-mcp-server: rsync
# archivematica-mcp-client deps: software-properties-common ffmpeg fits jhove libxml2-dev pkg-config
...
RUN apt-get install -y apt-transport-https aptitude debconf-utils man time less nano emacs vim wget curl sudo openjdk-7-jre-headless openjdk-7-jdk software-properties-common python rsync gearman clamav-daemon nginx libxml2-dev libxslt-dev python-dev libmysqlclient-dev default-jdk python-dateutil python-gearman python-simplejson dbconfig-common libdbd-mysql-perl libdbi-perl libffi-dev libgsm1 libhtml-template-perl libopenjpeg2 liborc-0.4-0 libossp-uuid16 libschroedinger-1.0-0 libssl-dev libssl-doc libterm-readkey-perl libvo-aacenc0 libvo-amrwbenc0 libx264-142 libdbd-mysql-perl libdbi-perl libffi-dev libgsm1 libhtml-template-perl libopenjpeg2 liborc-0.4-0 libossp-uuid16 libschroedinger-1.0-0 libssl-dev libssl-doc libaio1 uuid

# Add archivematica, archivematica-mcp-client deps mc3man, elasticsearch repo:
RUN \
echo "deb [arch=amd64] https://packages.archivematica.org/1.6.x/ubuntu trusty main" >> /etc/apt/sources.list.d/archivematica-repo.list && \
echo "deb [arch=amd64] https://packages.archivematica.org/1.6.x/ubuntu-externals trusty main" >> /etc/apt/sources.list.d/archivematica-repo.list && \
wget -O - https://packages.archivematica.org/1.6.x/key.asc | apt-key add - && \
add-apt-repository ppa:mc3man/trusty-media && \
echo "deb http://packages.elasticsearch.org/elasticsearch/1.7/debian stable main" >> /etc/apt/sources.list.d/elasticsearch-repo.list && \
wget -O - http://packages.elasticsearch.org/GPG-KEY-elasticsearch | apt-key add - && \
apt-get update

# Install elasticsearch, a ton of deps for archivematica-mcp-client
RUN apt-get install -y unar ffmpeg fits jhove bagit atool bulk-extractor clamav sleuthkit tesseract-ocr tika tree ufraw unrar-free p7zip-full pbzip2 libimage-exiftool-perl imagemagick inkscape mediainfo nailgun-client keyutils readpst rpcbind siegfried postfix libxml2-utils logapp md5deep nfs-common libvo-aacenc0 libvo-amrwbenc0 libx264-142 libavcodec-extra-56 libavutil54 libavcodec-extra-56 libswresample1 
